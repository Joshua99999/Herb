...
const EmployeePayrollStructure = () => {
    ...
    const [payrollData, setPayrollData] = useState({ EMPS: [] });
    ...
    useEffect(() => {
        ...
        PayrollService.selectedEmp(params).then((res) => {
        ...
        PayrollService.addEarnings().then((res) => {
            if (res.data) {
                let eOptions = []
    ...
    const FnReset = () => {
        ...
        PayrollService.resetStructure(params).then((res) => {
            setLoading(false)
            if (res.data.SCODE === "01") {
    ...
    const FnSave = () => {
        ...
        PayrollService.SaveStructure(params).then((res) => {
            setLoading(false)
            if (res.data.SCODE === "01") {
                toast.success(res.data.SDESC , {autoClose : false})
                setEmpData(res.data)
    ...
    return (
        <>
                                                ...
                                                onChange={selectedOption => {
                                                    ...
                                                    if (selectedOption !== null) {
                                                        ...
                                                        if (selectedOption.calcType !== "MANUAL") {
                                                            ...
                                                            PayrollService.getEarningAmout(params).then(res => {
                                                                if (res.status === 200) {
            ...
            <Modal show={showEarningAttachment} onHide={popupClose}
                // dialogClassName="modal-container"
                dialogClassName="modal-90w"
                size="lg"
                keyboard
