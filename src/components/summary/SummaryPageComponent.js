...
const SummaryPagecomponent = () => {
  ...
  useEffect(() => {
    ...
    PayrollService.getCADRE(params).then((res) => {
      setData(res.data)
    })
  }, [location]);
  ...
  const attendanceSave = () => {
    ...
    PayrollService.attendanceSave(attendanceParams).then((res) => {
  ...
  const attendanceDisplay = (CFMSID, i) => {
    ...
    setCfmsId(CFMSID)
    let attendanceParams = { "cfmsId": CFMSID, "ddocode": location.state.ddocode, "hoa": location.state.hoa, "year": location.state.year, "month": location.state.month }
    PayrollService.attendanceDisplay(attendanceParams).then((res) => {
      if (res.data) {
